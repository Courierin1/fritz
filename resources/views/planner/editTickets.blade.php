...
<style>
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                @include('alerts')
                <form id="ticket" method="POST" action="{{route('user.edit.event.tickets.post')}}" >
                    @csrf
                    <div class="nav-tabs-custom">
                            <h2 class="page-header">Tickets</h2>
                        <ul class="nav nav-pills">
                            <li class="get_ticket_type  {{ ((old('ticket_type') ?? $event->ticket_type) == 'paid') ? 'active' : '' }}"
                                data-ticket="paid"><a href="#paid" data-toggle="tab"
                                aria-expanded="true">Paid</a>

                            <input type="hidden" name="ticket_type" value="{{old('ticket_type') ?? $event->ticket_type}}" class="tclass" >
                            </li>
                            <li class="get_ticket_type {{ ((old('ticket_type') ?? $event->ticket_type) == 'free') ? 'active' : '' }}" data-ticket="free"><a href="#free" data-toggle="tab"
                                    aria-expanded="false">Free</a></li>
                            <li class="get_ticket_type  {{ ((old('ticket_type') ?? $event->ticket_type) == 'donation') ? 'active' : '' }}" data-ticket="donation"><a href="#donation" data-toggle="tab"
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="paid">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="exampleInputEmail1" required name="name" value="{{old('name') ?? $event->name}}"
                                        placeholder="Ticket Name">
                                </div>

                                <div class="form-group">
                                    <label for="exampleInputEmail1">Available Quantity*</label>
                                    <input   placeholder="Enter Quantity"  type="number" name="available_quantity" id="available_quantity" class="form-control" required value="{{old('available_quantity') ?? $event->available_quantity}}"
                                        id="exampleInputEmail1">
                                </div>

                                <div class="form-group price {{((old('ticket_type') ?? $event->ticket_type) != 'paid') ? 'd_none' : ''}}">
                                    <label for="price">Price*</label>
                                    <input type="text" class="form-control" name="price" id="price" value="{{old('price') ?? $event->price}}" step="1" min="1" pattern="^\d*(\.\d{0,2})?$"
                                        placeholder="$ 0.00">
                                </div>
                                      ...
                                      <div class="form-group">
                                          ...
                                          <div class="input-group date">
                                              <input type="date" id="sale_start"
                                                  ...
                                                  name="sale_start"
                                                  value="{{ old('sale_start') ?? $event->sale_start }}">
                                          </div>
                                      </div>
                                  </div>
                                  <div class="col-md-6">
                                          ...
                                          <div class="form-group">
                                              ...
                                              <div class="input-group">
                                                  <input type="time" id="sale_start_time" class="form-control"
                                                      ...
                                                      value="{{ old('sale_start_time') ?? $event->sale_start_time }}">
                                              </div>
                                          </div>
                                      </div>
                                  </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group date">
                                                ...
                                                <input type="date" id="sale_end" class="form-control pull-right"
                                                    ...
                                                    min="{{date('Y-m-d')}}"
                                                    max="{{date('Y-m-d', strtotime($event->event_start))}}"
                                                    value="{{ old('sale_end') ?? $event->sale_end }}">
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="input-group">
                                                    <input type="time" id="sale_end_time" class="form-control"
                                                        ...
                                                        value="{{ old('sale_end_time') ?? $event->sale_end_time }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" rows="3" name="description" value="{{$event->description}}"
                                        placeholder="Tell attendees more about this ticket.">{{old('description') ?? $event->description}}</textarea>
                                </div>

                                <div class="form-group maxticket {{((old('ticket_type') ?? $event->ticket_type) == 'donation') ? 'd_none' : ''}}">
                                    <label for="exampleInputEmail1">Max Tickets Per Order(*)</label>
                                    <input placeholder="Max Tickets (10)" type="number" class="form-control" name="max_ticket" id="max_ticket" value="{{old('max_ticket') ?? $event->max_ticket}}"
                                        id="exampleInputEmail1">
                                </div>

                                <div class="form-group minticket {{((old('ticket_type') ?? $event->ticket_type) == 'donation') ? 'd_none' : ''}}">
                                    <label for="exampleInputEmail1">Min Tickets Per Order(*)</label>
                                    <input  placeholder="Min Tickets(1)" type="number" class="form-control" name="min_ticket" id="min_ticket" value="{{old('min_ticket') ?? $event->min_ticket}}"
                                        id="exampleInputEmail1">
                                </div>
                    ...
                    <div class="box-footer" >
						<button style="margin-top:3%;" type="submit" class="mt-5 pull-right" id="pop1"><i class="fa-solid fa-arrow-right"></i></button>
